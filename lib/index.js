 ...
 * with fetch + JSON bodies. All operations are read-only:
 *
 ...
 * back to the `shell` service. Nothing is ever written, committed, or
 * checked out.
 */
...
export function apply(ctx) {
  ...
  function summarizeSessionTurns(events) {
    ...
    const addFileDelta = (turn, path, added, removed, op) => {
      ...
      if (event.type === 'turn/start') {
        ...
        if (args && typeof args === 'object' && typeof d.turn === 'number') {
          const path = (typeof args.file_path === 'string' && args.file_path !== '') ? args.file_path : ((typeof args.path === 'string' && args.path !== '') ? args.path : '')
            if (path) {
              if (d.name === 'write') addFileDelta(d.turn, path, lineCount(args.content), 0, { op: 'write', turn: d.turn, content: typeof args.content === 'string' ? args.content : '' })
              else if (d.name === 'edit') addFileDelta(d.turn, path, lineCount(args.new_string), lineCount(args.old_string), { op: 'edit', turn: d.turn, oldStr: typeof args.old_string === 'string' ? args.old_string : '', newStr: typeof args.new_string === 'string' ? args.new_string : '', replaceAll: !!(args.replace_all === true) })
              else if (d.name === 'str_replace_editor') {
                if (args.command === 'create') addFileDelta(d.turn, path, lineCount(args.file_text), 0, { op: 'write', turn: d.turn, content: typeof args.file_text === 'string' ? args.file_text : '' })
                else if (args.command === 'str_replace') addFileDelta(d.turn, path, lineCount(args.new_str), lineCount(args.old_str), { op: 'edit', turn: d.turn, oldStr: typeof args.old_str === 'string' ? args.old_str : '', newStr: typeof args.new_str === 'string' ? args.new_str : '', replaceAll: !!(args.replace_all === true) })
                else if (args.command === 'insert') addFileDelta(d.turn, path, lineCount(args.text), 0, { op: 'insert', turn: d.turn, line: args.insert_line, text: typeof args.text === 'string' ? args.text : (typeof args.new_str === 'string' ? args.new_str : '') })
              }
            }
            }
