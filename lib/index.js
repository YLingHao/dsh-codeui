...
export function apply(ctx) {
  ...
  function summarizeSessionTurns(events) {
    ...
    const addFileDelta = (turn, path, added, removed) => {
      ...
      if (!row) { row = { path: path, added: 0, removed: 0 }; cell.map.set(path, row) }
      row.added += added
      row.removed += removed
      cell.added += added
    ...
    for (const event of events) {
      ...
      if (event.type === 'turn/start') {
        ...
        if (args && typeof args === 'object' && typeof d.turn === 'number') {
          ...
          if (path) {
            if (d.name === 'write') addFileDelta(d.turn, path, lineCount(args.content), 0)
            else if (d.name === 'edit') addFileDelta(d.turn, path, lineCount(args.new_string), lineCount(args.old_string))
            else if (d.name === 'str_replace_editor') {
              if (args.command === 'create') addFileDelta(d.turn, path, lineCount(args.file_text), 0)
              else if (args.command === 'str_replace') addFileDelta(d.turn, path, lineCount(args.new_str), lineCount(args.old_str))
              else if (args.command === 'insert') addFileDelta(d.turn, path, lineCount(args.text), 0)
            }
          }
        }
