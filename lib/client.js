...
function DiffPanel(props) {
  ...
  const fileRows = changedPaths.map(function (path) {
    ...
    const delta = fileTurnDelta(path, key, filesMap)
    let added = null
    let removed = null
  ...
  if (cumulative != null) {
    ...
    if (isChanged && loading) {
      ...
      const delta = key != null ? fileTurnDelta(activeTab.path, key, filesMap) : null
      ...
      if (d) {
        ...
        const mode = viewMode
        if (mode === 'before') body = useToolDelta ? renderToolDeltaView(activeTab.path, key, filesMap) : renderFullText(d.before, activeTab.path + ' · ' + tr('diff_turn_before', { n: key }))
        else if (mode === 'after') body = useToolDelta ? renderToolDeltaView(activeTab.path, key, filesMap) : renderFullText(d.after, activeTab.path + ' · ' + tr('diff_turn_after', { n: key }))
        else if (mode === 'current') body = vm && vm.current != null ? renderFullText(vm.current, activeTab.path + ' · ' + tr('diff_current_disk')) : renderToolDeltaView(activeTab.path, key, filesMap)
        else if (mode === 'unified') {
          const r = useToolDelta ? renderToolDeltaRows(activeTab.path, key, filesMap) : renderUnifiedRows(d)
          const shown = delta || r
          body = h('div', { className: 'codeui-diff-content' },
          ...
          const r = useToolDelta ? renderToolDeltaRows(activeTab.path, key, filesMap) : renderSplitRows(d, splitRatio, beginSplitDrag)
          const shown = delta || r
          body = h('div', { className: 'codeui-diff-content' },
            h('div', { className: 'codeui-diff-filepath', title: activeTab.path }, activeTab.path + tr('diff_turn_suffix', { n: key })),
            h('div', { className: 'codeui-diff-stats' },
