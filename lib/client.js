...
const STRINGS = {
  'zh-CN': {
    ...
    diff_unified_compact_title: '显示简略统一差异',
    diff_unified_full_title: '显示完整统一差异',
    diff_view_before: '改前',
    diff_view_after: '改后',
    ...
    editor_loading: '正在读取当前文件…',
    editor_unsaved: '未保存',
    editor_saving: '正在保存…',
    editor_saved: '已自动保存',
    editor_save_error: '保存失败：{error}',
    editor_sandbox: '保存失败：文件不在当前工作区的可写范围内。',
    editor_stale: '文件已被其他程序或智能体修改，请重新载入后再编辑。',
    editor_reload: '重新载入',
    editor_hint: '自动保存 · Ctrl+S 立即保存',
    editor_mode_title: '编辑当前磁盘文件（自动保存）',
  ...
  en: {
    ...
    diff_unified_compact_title: 'Show compact unified diff',
    diff_unified_full_title: 'Show full unified diff',
    ...
    diff_unmodified_lines: '{n} unmodified lines',
    review_card_edited: 'Edited {n} files',
    review_card_button: 'Review',
    ...
    editor_loading: 'Reading current file…',
    editor_unsaved: 'Unsaved',
    editor_saving: 'Saving…',
    editor_saved: 'Autosaved',
    editor_save_error: 'Save failed: {error}',
    editor_sandbox: 'Save failed: the file is outside the writable workspace.',
    editor_stale: 'The file changed in another program or agent. Reload before editing again.',
    editor_reload: 'Reload',
    editor_hint: 'Autosave · Ctrl+S to save now',
    editor_mode_title: 'Edit current disk file (autosave)',
    settings_title: 'Code UI',
...
function reviewOpKey(op) {
  if (!op) return ''
  return [op.turn, op.op, op.line, op.replaceAll ? 1 : 0, op.content, op.oldStr, op.newStr, op.text].map(function (v) { return String(v == null ? '' : v) }).join('|')
}

// Session event times and persisted history do not always use the same clock.
// Turn number is the stable primary order; sequence/time only order operations
// within a turn.
function compareReviewOps(a, b) {
  const at = typeof a?.turn === 'number' ? a.turn : 0
  const bt = typeof b?.turn === 'number' ? b.turn : 0
  ...
  const ax = typeof a?.time === 'number' ? a.time : 0
  const bx = typeof b?.time === 'number' ? b.time : 0
  return ax - bx
}

function mergeTurnFileLists(historyFiles, liveFiles, turn) {
  const byPath = new Map()
  const add = function (row, sourceIndex) {
    if (!row || typeof row.path !== 'string') return
    const rowKey = mergePathKey(row.path, byPath)
    let out = byPath.get(rowKey)
    if (!out) {
      out = Object.assign({}, row, { ops: [] })
      byPath.set(rowKey, out)
    }
    const ops = Array.isArray(row.ops) ? row.ops : []
    for (let i = 0; i < ops.length; i++) {
      const op = Object.assign({}, ops[i], {
        turn: typeof ops[i].turn === 'number' ? ops[i].turn : turn,
        order: typeof ops[i].order === 'number' ? ops[i].order : (sourceIndex * 100000 + i),
      })
      const key = reviewOpKey(op)
      if (!out.ops.some(function (existing) { return reviewOpKey(existing) === key })) out.ops.push(op)
    }
  }
  for (const row of Array.isArray(historyFiles) ? historyFiles : []) add(row, 0)
  for (const row of Array.isArray(liveFiles) ? liveFiles : []) add(row, 1)
  return Array.from(byPath.values()).map(function (row) {
    row.ops.sort(compareReviewOps)
    return row
  })
}
...
function collectFiles(snapshot) {
  ...
  function addOp(path, op, seq) {
    if (typeof path !== 'string' || path === '') return
    if (typeof op.seq !== 'number' && typeof seq === 'number') op.seq = seq
  ...
  function addToolModification(name, rawArgs, seq, time, fallbackTurn) {
    ...
    if (name === 'write') {
      addOp(path, { op: 'write', content: (typeof args.content === 'string') ? args.content : '', turn: turn, time: time }, seq)
    } else if (name === 'edit') {
      addOp(path, {
        op: 'edit',
      ...
      if (command === 'create') {
        addOp(path, { op: 'write', content: (typeof args.file_text === 'string') ? args.file_text : '', turn: turn, time: time }, seq)
        ...
        addOp(path, { op: 'insert', line: args.insert_line, text: (typeof args.new_str === 'string') ? args.new_str : '', turn: turn, time: time }, seq)
      }
    }
  }
  ...
  for (const f of map.values()) f.ops.sort(compareReviewOps)
  return map
}

function changedPathsForTurn(filesMap, turn, diffs) {
  const out = []
  for (const entry of filesMap) {
    const path = entry[0]
    ...
    if (hit && diffs && turn != null) {
      const vm = diffs[path] || diffs[mergePathKey(path, filesMap)]
      const d = vm && vm.turns ? vm.turns[turn] : null
      if (d && d.added === 0 && d.removed === 0) hit = false
    }
  ...
  for (const ht of h) {
    ...
    if (lt) {
      merged.push(Object.assign({}, ht, {
        ...
        files: mergeTurnFileLists(ht.files, lt.files, ht.turn),
...
// Bounded line diff for large files. Unique common lines act as anchors so a
// large edit does not degrade into an inaccurate delete-all/add-all block.
function patienceOps(a, b) {
  const left = new Map()
  const right = new Map()
  for (let i = 0; i < a.length; i++) {
    const n = left.get(a[i])
    left.set(a[i], n ? { count: n.count + 1, index: n.index } : { count: 1, index: i })
  }
  for (let i = 0; i < b.length; i++) {
    const n = right.get(b[i])
    right.set(b[i], n ? { count: n.count + 1, index: n.index } : { count: 1, index: i })
  }
  const pairs = []
  for (const entry of left) {
    const r = right.get(entry[0])
    if (entry[1].count === 1 && r && r.count === 1) pairs.push({ a: entry[1].index, b: r.index })
  }
  ...
  while (at >= 0) { anchors.push(pairs[at]); at = prev[at] }
  anchors.reverse()
  const out = []
  const appendSegment = function (a0, a1, b0, b1) {
    const aa = a.slice(a0, a1)
    const bb = b.slice(b0, b1)
    if (!aa.length) { for (const text of bb) out.push({ t: 'a', text: text }); return }
    if (!bb.length) { for (const text of aa) out.push({ t: 'd', text: text }); return }
    if (aa.length * bb.length <= LCS_CELL_CAP) out.push.apply(out, lcsOps(aa, bb))
    else {
      for (const text of aa) out.push({ t: 'd', text: text })
      for (const text of bb) out.push({ t: 'a', text: text })
    }
  }
  let a0 = 0
  let b0 = 0
  for (const anchor of anchors) {
    appendSegment(a0, anchor.a, b0, anchor.b)
    out.push({ t: 'e', text: a[anchor.a] })
    a0 = anchor.a + 1
    b0 = anchor.b + 1
  }
  appendSegment(a0, a.length, b0, b.length)
  return out
}
...
function alignedDiffRows(oldText, newText) {
  ...
  if (canLcs) {
    ...
    for (const item of patienceOps(midA, midB)) flat.push(item)
  }
...
function syntaxLanguage(path) {
  ...
  if (/^(js|jsx|ts|tsx|mjs|cjs|json|css|scss|less|html|htm|vue|svelte|py|rb|go|rs|java|c|h|cpp|hpp|cs|php|sh|bash|yml|yaml|md)$/.test(ext)) return ext
  return 'text'
}

// Lightweight, dependency-free token coloring for the common source formats
// handled by the plugin. Text is still passed as React text, never as HTML.
function highlightCodeLine(text, path) {
  const source = String(text == null ? '' : text)
  if (syntaxLanguage(path) === 'text' || source === '') return source
  const token = /(<!--[\s\S]*?-->|<\/?[A-Za-z][^>]*>|\/\/[^\n]*|#(?=\s|$)[^\n]*|\/\*[\s\S]*?\*\/|"(?:\\.|[^"\\])*"|'(?:\\.|[^'\\])*'|`(?:\\.|[^`\\])*`|\b\d+(?:\.\d+)?\b|\b(?:const|let|var|function|return|if|else|for|while|do|switch|case|break|continue|class|new|import|from|export|default|async|await|try|catch|finally|throw|yield|extends|implements|interface|type|enum|namespace|readonly|abstract|static|public|private|protected|def|in|and|or|not|as|is|True|False|None|null|undefined|true|false)\b|\b[A-Za-z_$][\w$]*(?=\s*\()|\b[A-Z][A-Za-z0-9_$]*\b)/g
  const out = []
  let last = 0
  let match
  while ((match = token.exec(source))) {
    ...
    else if (/^<\/?[A-Za-z]/.test(value)) kind = 'tag'
    else if (/^(?:return|if|else|for|while|do|switch|case|break|continue|try|catch|finally|throw|yield|in|and|or|not|is)$/.test(value)) kind = 'control'
    else if (/^(?:const|let|var|function|class|new|import|from|export|default|async|await|extends|implements|interface|type|enum|namespace|readonly|abstract|static|public|private|protected|def|as)$/.test(value)) kind = 'keyword'
    else if (/^(?:True|False|None|null|undefined|true|false)$/.test(value)) kind = 'literal'
    else if (/^[A-Z]/.test(value)) kind = 'type'
    else if (/^[A-Za-z_$][\w$]*$/.test(value)) kind = 'function'
    out.push(h('span', { className: 'codeui-syntax-' + kind }, value))
...
function compactUnifiedRows(rows, context) {
  const radius = context == null ? 3 : context
  const changed = []
  for (let i = 0; i < rows.length; i++) if (rows[i].t !== 'ctx') changed.push(i)
  ...
  let i = 0
  while (i < rows.length) {
    if (keep.has(i)) { compact.push(rows[i]); i++; continue }
    const start = i
    while (i < rows.length && !keep.has(i)) i++
    compact.push({ t: 'fold', count: i - start })
  }
  return compact
}
...
async function computeVersionMap(path, ops) {
  ...
  const orderedOps = ops.slice().sort(compareReviewOps)

  let base
  if (orderedOps[0].op === 'write') {
    base = ''
  } else {
    base = current
    for (let i = orderedOps.length - 1; i >= 0; i--) {
      const o = orderedOps[i]
  ...
  let content = base
  let i = 0
  while (i < orderedOps.length) {
    const turn = orderedOps[i].turn
    const before = content
    while (i < orderedOps.length && orderedOps[i].turn === turn) {
      const o = orderedOps[i]
...
function versionSignature(filesMap) {
  ...
  for (const entry of filesMap) {
    const path = entry[0]
    const ops = entry[1].ops
    // Include operation content, not only timestamps. Historical operations
    // may be merged after the live snapshot with identical time boundaries;
    // a boundary-only cache key can then reuse a stale zero-line snapshot.
    parts.push(path + ':' + ops.map(reviewOpKey).join('~'))
  }
  return parts.join(';;')
}
...
function mergeHistoryFilesMap(filesMap, turnModel, turn) {
  ...
  if (!turnModel) return next
  const historyTurns = turn == null
    ? (Array.isArray(turnModel.turns) ? turnModel.turns : [])
    : (turnModel.turns.filter(function (t) { return t.turn === turn }))
  for (const ht of historyTurns) {
    if (!ht || !Array.isArray(ht.files)) continue
    for (const hf of ht.files) {
    if (!hf || typeof hf.path !== 'string') continue
    const key = mergePathKey(hf.path, next)
    const existing = next.get(key)
    // History rows are the authoritative source for unloaded turns.  Do not
    // deduplicate by turn: a loaded window may contain only part of a turn's
    // tool calls.  Deduplicate individual operations instead.
    const histOps = (Array.isArray(hf.ops) ? hf.ops : []).map(function (op, index) {
      return Object.assign({}, op, {
        turn: op && typeof op.turn === 'number' ? op.turn : ht.turn,
        order: op && typeof op.order === 'number' ? op.order : index,
        time: op && typeof op.time === 'number' ? op.time : ((ht.turn * 1000000) + index),
      })
    })
    if (existing) {
      const existingKeys = new Set(existing.ops.map(reviewOpKey))
      const mergedOps = existing.ops.slice()
      for (const op of histOps) if (op && !existingKeys.has(reviewOpKey(op))) { existingKeys.add(reviewOpKey(op)); mergedOps.push(op) }
      mergedOps.sort(compareReviewOps)
      next.set(key, Object.assign({}, existing, { ops: mergedOps }))
    } else {
      next.set(key, { ops: histOps, lastTime: 0, lastTurn: ht.turn, risk: { level: 'none', reasons: [] } })
    }
    }
  }
  return next
...
// Build the card counters from the same full-file snapshots used by Review.
// Tool argument line counts are useful as a fallback, but are not a diff:
// replacing a large block can contain many unchanged lines.
function buildTurnFileSummaryFromDiffs(diffs) {
  const byTurn = new Map()
  for (const entry of Object.entries(diffs || {})) {
    const path = entry[0]
    const vm = entry[1]
    if (!vm || !vm.turns) continue
    for (const turnKey of Object.keys(vm.turns)) {
      const turn = Number(turnKey)
      const d = vm.turns[turnKey]
      if (!d) continue
      // A tool may touch a file and then restore it in the same turn. It is
      // not a visible code change and should not become a +0/-0 card row.
      if (!d.added && !d.removed) continue
      let cell = byTurn.get(turn)
      if (!cell) { cell = { files: new Map(), added: 0, removed: 0 }; byTurn.set(turn, cell) }
      cell.files.set(path, { path: path, added: d.added || 0, removed: d.removed || 0 })
      cell.added += d.added || 0
      cell.removed += d.removed || 0
    }
  }
  const out = {}
  for (const entry of byTurn) {
    const files = Array.from(entry[1].files.values())
    files.sort(function (a, b) { return ((b.added + b.removed) - (a.added + a.removed)) || String(a.path).localeCompare(String(b.path)) })
    out[entry[0]] = { turn: entry[0], files: files, added: entry[1].added, removed: entry[1].removed }
  }
  return out
}
...
.codeui-diff{--codeui-editor-bg:#fff;--codeui-editor-fg:#000;--codeui-editor-gutter:#f3f3f3;--codeui-editor-line:#237893;--codeui-editor-selection:rgba(0,122,204,.24);height:100%;display:flex;flex-direction:column;overflow:hidden;background:var(--dsw-alias-bg-base,#fff);padding-right:0;position:relative;box-sizing:border-box}
[data-ds-dark-theme] .codeui-diff{--codeui-editor-bg:#1e1e1e;--codeui-editor-fg:#d4d4d4;--codeui-editor-gutter:#1e1e1e;--codeui-editor-line:#858585;--codeui-editor-selection:rgba(38,79,120,.68)}
...
.codeui-diff-scroll{flex:1;overflow:auto;background:var(--codeui-editor-bg);color:var(--codeui-editor-fg);font-family:var(--dsh-font-mono,ui-monospace,SFMono-Regular,Menlo,Consolas,monospace);font-size:12px;line-height:20px}
...
.codeui-diff-row{display:flex;min-width:max-content;border-left:3px solid transparent;box-sizing:border-box}
.codeui-diff-gutter{width:22px;flex:none;text-align:center;color:var(--dsw-alias-label-secondary,#999);user-select:none}
.codeui-diff-num{width:40px;flex:none;text-align:right;padding-right:10px;color:var(--codeui-editor-line);user-select:none}
.codeui-diff-text{white-space:pre;padding-right:16px}
.codeui-diff-ctx{background:transparent}
.codeui-diff-ctx .codeui-diff-text{color:var(--codeui-editor-fg)}
.codeui-diff-add{background:rgba(46,160,67,.14);border-left-color:#2ea043}
.codeui-diff-add .codeui-diff-gutter{color:#2ea043}
.codeui-diff-add .codeui-diff-text{color:var(--codeui-editor-fg)}
.codeui-diff-del{background:rgba(248,81,73,.14);border-left-color:#f85149}
.codeui-diff-del .codeui-diff-gutter{color:#f85149}
.codeui-diff-del .codeui-diff-text{color:var(--codeui-editor-fg)}
.codeui-diff-fold{display:flex;min-width:max-content;background:var(--dsw-alias-bg-layer-2,rgba(127,127,127,.07));border-top:1px solid var(--dsw-alias-border-l1,rgba(0,0,0,.06));border-bottom:1px solid var(--dsw-alias-border-l1,rgba(0,0,0,.06));color:var(--dsw-alias-label-secondary,#777)}
.codeui-diff-fold .codeui-diff-text{font-style:italic}
.codeui-syntax-comment{color:#008000 !important}
.codeui-syntax-string{color:#a31515 !important}
.codeui-syntax-number{color:#098658 !important}
.codeui-syntax-keyword{color:#0000ff !important}
.codeui-syntax-control{color:#af00db !important}
.codeui-syntax-literal{color:#0000ff !important}
.codeui-syntax-tag{color:#800000 !important}
.codeui-syntax-function{color:#795e26 !important}
.codeui-syntax-type{color:#267f99 !important}
[data-ds-dark-theme] .codeui-syntax-comment{color:#6a9955 !important}
[data-ds-dark-theme] .codeui-syntax-string{color:#ce9178 !important}
[data-ds-dark-theme] .codeui-syntax-number{color:#b5cea8 !important}
[data-ds-dark-theme] .codeui-syntax-keyword{color:#569cd6 !important}
[data-ds-dark-theme] .codeui-syntax-control{color:#c586c0 !important}
[data-ds-dark-theme] .codeui-syntax-literal{color:#569cd6 !important}
[data-ds-dark-theme] .codeui-syntax-tag{color:#569cd6 !important}
[data-ds-dark-theme] .codeui-syntax-function{color:#dcdcaa !important}
[data-ds-dark-theme] .codeui-syntax-type{color:#4ec9b0 !important}
...
.codeui-split-cell{display:flex;width:100%;min-width:0;border-left:3px solid transparent;box-sizing:border-box}
...
.codeui-split-del{background:rgba(248,81,73,.14);border-left-color:#f85149}
.codeui-split-add{background:rgba(46,160,67,.14);border-left-color:#2ea043}
.codeui-split-del .codeui-diff-gutter{color:#f85149}
.codeui-split-add .codeui-diff-gutter{color:#2ea043}
.codeui-split-del .codeui-diff-text,.codeui-split-add .codeui-diff-text{color:var(--codeui-editor-fg)}
...
.codeui-textnum{display:table-cell;width:44px;padding-right:10px;text-align:right;color:var(--codeui-editor-line);user-select:none}
.codeui-textline{display:table-cell;white-space:pre;padding-right:16px;color:var(--codeui-editor-fg)}

.codeui-editor-content{background:var(--codeui-editor-bg)}
.codeui-editor-filebar{display:flex;align-items:center;gap:10px;min-height:32px;padding:4px 10px;border-bottom:1px solid var(--dsw-alias-border-l1,rgba(0,0,0,.10));background:var(--dsw-alias-bg-layer-1,#fff);font-size:11px;box-sizing:border-box}
.codeui-editor-path{flex:1;min-width:0;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;color:var(--dsw-alias-label-primary,#222);font-family:var(--dsh-font-mono,ui-monospace,SFMono-Regular,Menlo,Consolas,monospace)}
.codeui-editor-hint{flex:none;color:var(--dsw-alias-label-tertiary,#999)}
.codeui-editor-status{flex:none;max-width:280px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;color:var(--dsw-alias-label-secondary,#777)}
.codeui-editor-status-unsaved{color:#bf8803}
.codeui-editor-status-saving{color:#007acc}
.codeui-editor-status-saved{color:#16825d}
.codeui-editor-status-error,.codeui-editor-status-stale{color:#f14c4c}
.codeui-editor-main{position:relative;flex:1;min-height:0;overflow:hidden;background:var(--codeui-editor-bg);color:var(--codeui-editor-fg);font-family:var(--dsh-font-mono,ui-monospace,SFMono-Regular,Menlo,Consolas,monospace);font-size:13px;line-height:20px}
.codeui-editor-gutter{position:absolute;z-index:3;left:0;top:0;bottom:0;width:54px;overflow:hidden;background:var(--codeui-editor-gutter);border-right:1px solid rgba(127,127,127,.12);pointer-events:none;user-select:none}
.codeui-editor-line-numbers{padding:8px 10px 32px 0;box-sizing:border-box;will-change:transform}
.codeui-editor-line-number{height:20px;text-align:right;color:var(--codeui-editor-line);font-variant-numeric:tabular-nums}
.codeui-editor-highlight{position:absolute;z-index:1;left:54px;top:0;margin:0;min-width:max-content;min-height:100%;padding:8px 16px 32px 12px;box-sizing:border-box;color:var(--codeui-editor-fg);font:inherit;line-height:20px;tab-size:4;pointer-events:none;will-change:transform}
.codeui-editor-code-line{height:20px;white-space:pre}
.codeui-editor-input{position:absolute;z-index:2;left:54px;right:0;top:0;bottom:0;width:calc(100% - 54px);height:100%;box-sizing:border-box;margin:0;padding:8px 16px 32px 12px;resize:none;overflow:auto;border:0;outline:0;background:transparent;color:transparent;-webkit-text-fill-color:transparent;caret-color:var(--codeui-editor-fg);font:inherit;line-height:20px;tab-size:4;white-space:pre}
.codeui-editor-input::selection{background:var(--codeui-editor-selection)}
.codeui-editor-input[readonly]{cursor:not-allowed}
@media (max-width:800px){.codeui-editor-hint{display:none}.codeui-editor-status{max-width:130px}.codeui-editor-filebar{gap:6px}}
...
function renderUnifiedRows(d, path, expanded) {
  const rows = expanded ? alignedDiffRows(d.before, d.after) : compactUnifiedRows(alignedDiffRows(d.before, d.after), 3)
  let added = 0
  let removed = 0
  const rowEls = []
  const push = function (r, side) {
    ...
    const sign = isAdd ? '+' : '-'
    const lineNum = isAdd ? (r.rn == null ? '' : String(r.rn)) : (r.ln == null ? '' : String(r.ln))
    ...
    rowEls.push(h('div', { className: 'codeui-diff-row ' + cls, key: rowEls.length },
      ...
      h('span', { className: 'codeui-diff-num' }, lineNum),
      h('span', { className: 'codeui-diff-text' }, text === '' ? '\u00a0' : highlightCodeLine(text, path))
    ))
  }
  for (const r of rows) {
    if (r.t === 'fold') {
      rowEls.push(h('div', { className: 'codeui-diff-fold', key: 'fold' + rowEls.length },
        h('span', { className: 'codeui-diff-gutter' }, ' '),
        h('span', { className: 'codeui-diff-num' }, '...'),
        h('span', { className: 'codeui-diff-text' }, tr('diff_unmodified_lines', { n: r.count }))
      ))
      continue
    }
    if (r.t === 'ctx') {
      rowEls.push(h('div', { className: 'codeui-diff-row codeui-diff-ctx', key: rowEls.length },
        ...
        h('span', { className: 'codeui-diff-text' }, r.ltext === '' ? '\u00a0' : highlightCodeLine(r.ltext, path))
      ))
    } else if (r.t === 'mod') {
      push(r, 'del')
      push(r, 'add')
...
function splitSide(row, side, path) {
  const isLeft = side === 'l'
...
function renderSplitRows(d, splitRatio, onSplitDrag, path) {
  ...
  for (let i = 0; i < rows.length; i++) {
    leftRows.push(h('div', { className: 'codeui-split-row', key: 'l' + i }, splitSide(rows[i], 'l', path)))
    rightRows.push(h('div', { className: 'codeui-split-row', key: 'r' + i }, splitSide(rows[i], 'r', path)))
  }
...
function renderFullText(content, label, path) {
  const lines = splitLines(content)
  const rows = []
  for (let i = 0; i < lines.length; i++) {
    rows.push(h('div', { className: 'codeui-textrow', key: i },
      h('span', { className: 'codeui-textnum' }, String(i + 1)),
      h('span', { className: 'codeui-textline' }, lines[i] === '' ? '\u00a0' : highlightCodeLine(lines[i], path || label))
    ))
  }
  return h('div', { className: 'codeui-diff-content' },
...
function cacheEditorContent(path, content) {
...
function EditableCodeEditor(props) {
  ...
  function setEditorStatus(next, detail) {
  ...
  async function flushSave() {
    ...
    if (!versionRef.current || conflictRef.current || valueRef.current === savedRef.current) return
    ...
    let res
    try { res = await hostCall('writeFile', { path: path, content: content, version: version, workspaceRoot: props.workspaceRoot || '' }) }
    catch (e) { res = { ok: false, error: String((e && e.message) || e) } }
    savingRef.current = false
    if (res && res.ok && typeof res.version === 'string') {
      const savedContent = typeof res.content === 'string' ? res.content : content
      versionRef.current = res.version
      savedRef.current = savedContent
      cacheEditorContent(path, savedContent)
      if (typeof props.onSaved === 'function') props.onSaved(path, savedContent)
      if (valueRef.current === savedContent) setEditorStatus('saved')
      else setEditorStatus('unsaved')
      if (queuedRef.current || valueRef.current !== savedRef.current) flushSave()
      return
    }
    const detail = String((res && res.error) || 'unknown-error')
    if (detail === 'FS_STALE_VERSION') {
      conflictRef.current = true
      setEditorStatus('stale', detail)
    } else if (detail === 'FS_SANDBOX_DENIED') {
      setEditorStatus('sandbox', detail)
    } else {
      setEditorStatus('error', detail)
    }
  }

  function scheduleSave() {
    if (timerRef.current != null) clearTimeout(timerRef.current)
    timerRef.current = setTimeout(function () { timerRef.current = null; flushSave() }, 600)
  }
  ...
  function loadCurrent() {
    ...
    hostCall('readFile', { path: path }).then(function (res) {
      if (!mountedRef.current) return
      if (!res || !res.ok || typeof res.content !== 'string' || typeof res.version !== 'string') {
        setEditorStatus('error', String((res && res.error) || 'read-failed'))
        return
      }
      valueRef.current = res.content
      savedRef.current = res.content
      versionRef.current = res.version
      setValue(res.content)
      setLoaded(true)
      setEditorStatus('saved')
      cacheEditorContent(path, res.content)
    }).catch(function (e) {
      setEditorStatus('error', String((e && e.message) || e))
    })
  }
  ...
  React.useEffect(function () {
    if (loaded && inputRef.current && typeof inputRef.current.focus === 'function') inputRef.current.focus()
  }, [loaded])

  function changeValue(next) {
    valueRef.current = next
    setValue(next)
    if (next === savedRef.current) setEditorStatus('saved')
    else setEditorStatus('unsaved')
    scheduleSave()
  }
  ...
  function onKeyDown(e) {
    ...
    if (ctrl && String(e.key || '').toLowerCase() === 's') {
      e.preventDefault()
      flushSave()
      return
    }
    if (e.key !== 'Tab' || conflictRef.current) return
    e.preventDefault()
    const input = e.currentTarget
    const start = input.selectionStart
    const end = input.selectionEnd
    const next = valueRef.current.slice(0, start) + '\t' + valueRef.current.slice(end)
    changeValue(next)
    if (typeof requestAnimationFrame === 'function') {
      requestAnimationFrame(function () {
  ...
  function syncScroll(e) {
    const input = e.currentTarget
    if (highlightRef.current) highlightRef.current.style.transform = 'translate(' + (-input.scrollLeft) + 'px,' + (-input.scrollTop) + 'px)'
    if (numbersRef.current) numbersRef.current.style.transform = 'translateY(' + (-input.scrollTop) + 'px)'
  }

  const lines = value === '' ? [''] : splitLines(value)
  const numberRows = []
  const codeRows = []
  for (let i = 0; i < lines.length; i++) {
    numberRows.push(h('div', { className: 'codeui-editor-line-number', key: 'n' + i }, String(i + 1)))
    codeRows.push(h('div', { className: 'codeui-editor-code-line', key: 'c' + i }, lines[i] === '' ? '\u00a0' : highlightCodeLine(lines[i], path)))
  }
  let statusText = tr('editor_saved')
  if (status === 'loading') statusText = tr('editor_loading')
  else if (status === 'unsaved') statusText = tr('editor_unsaved')
  else if (status === 'saving') statusText = tr('editor_saving')
  else if (status === 'stale') statusText = tr('editor_stale')
  else if (status === 'sandbox') statusText = tr('editor_sandbox')
  else if (status === 'error') statusText = tr('editor_save_error', { error: error || 'unknown-error' })
  const canReload = status === 'stale' || status === 'error'
  return h('div', { className: 'codeui-diff-content codeui-editor-content' },
    h('div', { className: 'codeui-editor-filebar' },
      h('span', { className: 'codeui-editor-path', title: path }, path),
      h('span', { className: 'codeui-editor-hint' }, tr('editor_hint')),
      h('span', { className: 'codeui-editor-status codeui-editor-status-' + status, 'aria-live': 'polite' }, statusText),
      canReload ? h('button', { className: 'codeui-btn', type: 'button', onClick: loadCurrent }, tr('editor_reload')) : null
    ),
    loaded ? h('div', { className: 'codeui-editor-main' },
      h('div', { className: 'codeui-editor-gutter' }, h('div', { className: 'codeui-editor-line-numbers', ref: numbersRef }, numberRows)),
      h('pre', { className: 'codeui-editor-highlight', ref: highlightRef, 'aria-hidden': 'true' }, codeRows),
      h('textarea', {
        className: 'codeui-editor-input',
        ref: inputRef,
        value: value,
    ...
    ) : h('div', { className: 'codeui-diff-empty' }, statusText)
  )
}
...
function TurnSync(props) {
  ...
  const [exactTurnFiles, setExactTurnFiles] = React.useState({})
  ...
  React.useEffect(function () {
    let cancelled = false
    if (!sessionId) { setExactTurnFiles({}); return }
    setExactTurnFiles({})
    const effectiveMap = mergeHistoryFilesMap(filesMap, merged, null)
    const paths = Array.from(effectiveMap.keys())
    Promise.all(paths.map(function (path) {
      return computeVersionMap(path, effectiveMap.get(path).ops).then(function (r) { return [path, r] })
    })).then(function (rows) {
      if (cancelled) return
      const maps = {}
      for (const row of rows) maps[row[0]] = row[1]
      setExactTurnFiles(buildTurnFileSummaryFromDiffs(maps))
    }).catch(function () { if (!cancelled) setExactTurnFiles({}) })
    return function () { cancelled = true }
  }, [sessionId, filesMap, merged])
  const turnFiles = exactTurnFiles
  React.useEffect(function () {
...
function DiffPanel(props) {
  ...
  // Reconstruct every selected turn from the complete session operation set.
  // Merging only the selected historical turn loses the edits between that
  // turn and the current disk state, which makes before/after snapshots and
  // line numbers differ depending on the entry point.
  const effectiveFilesMap = React.useMemo(function () { return mergeHistoryFilesMap(filesMap, turnModel, null) }, [filesMap, turnModel])
  React.useEffect(function () { setUnifiedExpanded(false) }, [shared.activePath, effectiveTurn])
  ...
  const changedPaths = changedPathsForTurn(effectiveFilesMap, displayTurn, diffs).sort(function (a, b) {
  ...
  function afterEditorSave(path, content) {
    const key = mergePathKey(path, effectiveFilesMap)
    setDiffs(function (prev) {
      const vm = prev[key] || prev[path]
      if (!vm) return prev
      const next = Object.assign({}, prev)
      next[key] = Object.assign({}, vm, { current: content })
      versionCache.maps = next
      return next
    })
    if (shared.settings.showGit && shared.workspaceRoot) loadGitStatus(shared.workspaceRoot)
  }
  ...
  const fileRows = changedPaths.map(function (path) {
    ...
    const vm = diffs[mergePathKey(path, effectiveFilesMap)] || diffs[path]
    const key = displayTurn != null ? displayTurn : (f ? f.lastTurn : null)
    let added = null
    let removed = null
    if (vm && key != null && vm.turns[key]) {
  ...
  if (cumulative != null) {
    ...
    if (viewMode === 'edit') {
      body = h(EditableCodeEditor, { key: 'editor-' + activeTab.path + '-' + shared.workspaceRoot, path: activeTab.path, workspaceRoot: shared.workspaceRoot, onSaved: afterEditorSave })
    } else {
    ...
    if (isChanged && loading) {
      ...
      // Always render from the complete reconstructed snapshot so line
      // numbers stay relative to the whole file, including old turns.
      if (d) {
        const mode = viewMode
        if (mode === 'before') body = renderFullText(d.before, activeTab.path + ' · ' + tr('diff_turn_before', { n: key }), activeTab.path)
        else if (mode === 'after') body = renderFullText(d.after, activeTab.path + ' · ' + tr('diff_turn_after', { n: key }), activeTab.path)
        else if (mode === 'current') body = vm && vm.current != null ? renderFullText(vm.current, activeTab.path + ' · ' + tr('diff_current_disk'), activeTab.path) : renderFullText(d.after, activeTab.path + ' · ' + tr('diff_turn_after', { n: key }), activeTab.path)
        else if (mode === 'unified') {
          const r = renderUnifiedRows(d, activeTab.path, unifiedExpanded)
          body = h('div', { className: 'codeui-diff-content' },
          ...
          const r = renderSplitRows(d, splitRatio, beginSplitDrag, activeTab.path)
          body = h('div', { className: 'codeui-diff-content' },
            h('div', { className: 'codeui-diff-filepath', title: activeTab.path }, activeTab.path + tr('diff_turn_suffix', { n: key })),
            h('div', { className: 'codeui-diff-stats' },
              h('span', { className: 'codeui-add-count' }, '+' + r.added + ' ' + tr('diff_added')),
      ...
      body = renderFullText(activeTab.content, activeTab.path + ' · ' + tr('diff_preview'), activeTab.path)
    } else {
  ...
  return h('div', { className: 'codeui-diff' },
    ...
    h('div', { className: 'codeui-toolbar' },
      ...
      h('span', { className: 'codeui-seg' },
        h('button', { className: 'codeui-seg-btn' + (viewMode === 'unified' ? ' codeui-seg-on' : ''), 'aria-pressed': viewMode === 'unified' ? 'true' : 'false', onClick: function () { if (viewMode === 'unified') setUnifiedExpanded(!unifiedExpanded); else { setViewMode('unified'); setUnifiedExpanded(false) } }, title: unifiedExpanded ? tr('diff_unified_compact_title') : tr('diff_unified_full_title') }, tr('diff_view_unified')),
        ...
        h('button', { className: 'codeui-seg-btn' + (viewMode === 'edit' ? ' codeui-seg-on' : ''), 'aria-pressed': viewMode === 'edit' ? 'true' : 'false', onClick: function () { setViewMode('edit') }, title: tr('editor_mode_title') }, tr('diff_view_edit'))
      ),
