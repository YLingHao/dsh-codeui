...
function renderToolDeltaRows(path, turn, filesMap) {
  ...
  const push = function (type, line) {
    if (type === 'add') { added++; els.push(h('div', { className: 'codeui-diff-row codeui-diff-add', key: 'a' + els.length }, h('span', { className: 'codeui-diff-gutter' }, '+'), h('span', { className: 'codeui-diff-text' }, line))) }
    else { removed++; els.push(h('div', { className: 'codeui-diff-row codeui-diff-del', key: 'd' + els.length }, h('span', { className: 'codeui-diff-gutter' }, '-'), h('span', { className: 'codeui-diff-text' }, line))) }
  }
  if (f) {
    for (const op of f.ops) {
...
function mergeHistoryFilesMap(filesMap, turnModel, turn) {
  ...
  for (const hf of ht.files) {
    ...
    const histOps = Array.isArray(hf.ops) ? hf.ops : []
    if (existing) {
...
function DiffPanel(props) {
  ...
  if (cumulative != null) {
    ...
    const isChanged = effectiveFilesMap.has(activeTab.path)
    if (isChanged && loading) {
      ...
      const delta = key != null ? fileTurnDelta(activeTab.path, key, effectiveFilesMap) : null
      ...
      if (d) {
        ...
        const mode = viewMode
        if (mode === 'before') body = useToolDelta ? renderFullText(toolDeltaText(activeTab.path, key, effectiveFilesMap, 'before'), activeTab.path + ' · ' + tr('diff_turn_before', { n: key })) : renderFullText(d.before, activeTab.path + ' · ' + tr('diff_turn_before', { n: key }))
        else if (mode === 'after') body = useToolDelta ? renderFullText(toolDeltaText(activeTab.path, key, effectiveFilesMap, 'after'), activeTab.path + ' · ' + tr('diff_turn_after', { n: key })) : renderFullText(d.after, activeTab.path + ' · ' + tr('diff_turn_after', { n: key }))
        else if (mode === 'current') body = vm && vm.current != null ? renderFullText(vm.current, activeTab.path + ' · ' + tr('diff_current_disk')) : renderToolDeltaView(activeTab.path, key, effectiveFilesMap)
        else if (mode === 'unified') {
          const r = useToolDelta ? renderToolDeltaRows(activeTab.path, key, effectiveFilesMap) : renderUnifiedRows(d)
          const shown = delta || r
          body = h('div', { className: 'codeui-diff-content' },
          ...
          const r = useToolDelta ? renderSplitRows({ before: toolDeltaText(activeTab.path, key, effectiveFilesMap, 'before'), after: toolDeltaText(activeTab.path, key, effectiveFilesMap, 'after') }, splitRatio, beginSplitDrag) : renderSplitRows(d, splitRatio, beginSplitDrag)
          const shown = delta || r
          body = h('div', { className: 'codeui-diff-content' },
            h('div', { className: 'codeui-diff-filepath', title: activeTab.path }, activeTab.path + tr('diff_turn_suffix', { n: key })),
            h('div', { className: 'codeui-diff-stats' },
