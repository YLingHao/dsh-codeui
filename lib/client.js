  ...
  if (cumulative != null) {
    ...
    if (isChanged && loading) {
      ...
      if (d) {
        ...
        const mode = viewMode
        if (mode === 'before') body = useToolDelta ? renderToolDeltaView(activeTab.path, key, effectiveFilesMap) : renderFullText(d.before, activeTab.path + ' · ' + tr('diff_turn_before', { n: key }))
        else if (mode === 'after') body = useToolDelta ? renderToolDeltaView(activeTab.path, key, effectiveFilesMap) : renderFullText(d.after, activeTab.path + ' · ' + tr('diff_turn_after', { n: key }))
