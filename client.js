...
function loadGitStatus(root) {
  ...
  host.call('codeui.gitStatus', { path: root }).then(function (res) {
    if (res && res.ok) {
...
.codeui-diff{height:100%;display:flex;flex-direction:column;overflow:hidden;background:var(--dsw-alias-bg-base,#fff);padding-right:30px}
...
.codeui-split-row{display:flex;min-width:max-content}
.codeui-split-pane{flex:0 0 auto;min-width:0;overflow-x:auto;overflow-y:hidden;position:relative}
.codeui-split-cell{display:flex;min-width:max-content}
...
function renderUnifiedRows(d) {
  const rows = diffRows(d.before, d.after)
  let added = 0
  let removed = 0
  const rowEls = []
  for (const r of rows) {
...
function TurnSync(props) {
  ...
  React.useEffect(function () {
    ...
    host.call('codeui.turnHistory', { sessionId: sessionId }).then(function (res) {
      if (cancelled) return
...
function JumpRail(props) {
  ...
  React.useEffect(function () {
    ...
    if (selected == null) {
      if (lastAutoTurn.current !== latestTurn) {
  ...
  if (rowHover) {
    ...
    if (ht) {
      const risk = ht.risk && ht.risk.level !== 'none'
      hoverCard = h('div', { className: 'codeui-rail-card', style: { top: rowHover.top + 'px', right: (rowHover.right != null ? rowHover.right : 390) + 'px' } },
        ...
        h('div', { className: 'codeui-rail-card-title' }, ht.userFull || ht.userTitle || ht.title || tr('turn_no_question')),
        h('div', { className: 'codeui-rail-card-meta' },
          ht.fileCount ? h('span', null, tr('files_changed_meta', { n: ht.fileCount })) : null,
          formatDuration(ht) ? h('span', null, formatDuration(ht)) : null,
          risk ? h('span', { className: 'codeui-rail-card-risk' }, '\u26a0 ' + (ht.risk.reasons || []).join('；')) : null
        )
      )
    }
  }
  ...
  return h('div', {
    ...
    h('div', {
      ...
      onMouseLeave: function () { setRailHover(false); setRowHover(null) },
    },
...
function DiffPanel(props) {
  ...
  React.useEffect(function () {
    ...
    Promise.all(paths.map(function (p) {
      return computeVersionMap(p, filesMap.get(p).ops).then(function (r) { return { path: p, r: r } }).catch(function () { return { path: p, r: { current: '', turns: {}, error: true } } })
  ...
  const changedPaths = changedPathsForTurn(filesMap, displayTurn).sort(function (a, b) {
    const fa = filesMap.get(a)
    const fb = filesMap.get(b)
    return ((fb && fb.lastTime) || 0) - ((fa && fa.lastTime) || 0)
  })

  function chooseAndJump(turnNumber) {
    chooseTurn(turnNumber, turnModel, true)
  }
  function selectTurnFromSelect(value) {
    chooseAndJump(value === 'latest' ? null : Number(value))
  }
  function stepTurn(delta) {
    ...
    if (next) chooseAndJump(next.turn)
  }
  ...
  function exportPatch() {
    ...
    host.call('codeui.gitDiff', { path: shared.workspaceRoot }).then(function (res) {
      if (res && res.ok) {
  ...
  const fileRows = changedPaths.map(function (path) {
    ...
    let added = null
    let removed = null
    const key = displayTurn != null ? displayTurn : latestTurnForFile(vm, null)
  ...
  if (cumulative != null) {
    ...
    const vm = diffs[activeTab.path]
    const loading = diffLoading[activeTab.path]
    ...
    if (isChanged && loading) {
      ...
      const d = key != null ? vm.turns[key] : null
      if (d) {
        const mode = viewMode
        if (mode === 'before') body = renderFullText(d.before, activeTab.path + ' · ' + tr('diff_turn_before', { n: key }))
        else if (mode === 'after') body = renderFullText(d.after, activeTab.path + ' · ' + tr('diff_turn_after', { n: key }))
        else if (mode === 'current') body = renderFullText(vm.current, activeTab.path + ' · ' + tr('diff_current_disk'))
        else if (mode === 'unified') {
          const r = renderUnifiedRows(d)
          body = h('div', { className: 'codeui-diff-content' },
          ...
          const r = renderSplitRows(d, splitRatio, beginSplitDrag)
          body = h('div', { className: 'codeui-diff-content' },
            h('div', { className: 'codeui-diff-filepath', title: activeTab.path }, activeTab.path + tr('diff_turn_suffix', { n: key })),
            h('div', { className: 'codeui-diff-stats' },
              h('span', { className: 'codeui-add-count' }, '+' + r.added + ' ' + tr('diff_added')),
...
function CodeUISettings() {
  ...
  return h('div', { className: 'codeui-settings' },
    ...
    row(tr('settings_reorder'), 'reorderLayout'),
